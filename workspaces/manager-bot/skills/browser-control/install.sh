...
VNC_PASSWORD=[REDACTED_SECRET] rand -base64 6)
...
VNC_PASSWORD=[REDACTED_SECRET] $SKILL_DIR/vnc-password 2>/dev/null || echo "")
...
if [ -n "$VNC_PASSWORD" ] && [ "$VNC_PASSWORD" != "(your Mac password)" ]; then
    NOVNC_URL="${TUNNEL_URL}/vnc.html?password=[REDACTED_SECRET]"
else
...
if [[ "$OS" == "mac" ]]; then
    ...
    echo "   2. Your VNC password = [REDACTED_SECRET] Mac login password"
    echo ""
fi
